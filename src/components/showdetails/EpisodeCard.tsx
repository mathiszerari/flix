    ...
    return ( 
        <div className="w-fit relative bg-gradient-to-t from-black to-transparent to-[50%] h-60 aspect-video flex items-end">
            ...
            <div className="p-4 flex flex-col backdrop-blur-sm w-full">
                <span className="text-lg">Episode {episode.episodeNumber} : {episode.title.length >= 20 ? episode.title.substring(0,20)+"..." : episode.title}</span>
                <span className="opacity-60">{episode.description.length >=50 ? episode.description.substring(0,50)+"..." : episode.description}</span>
